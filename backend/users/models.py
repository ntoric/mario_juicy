...
MENU_PERMISSIONS = {
    ...
    'parcel': ['restaurants.view_order', 'restaurants.add_order', 'restaurants.change_order'],
    'reservations': ['restaurants.view_reservation', 'restaurants.add_reservation', 'restaurants.change_reservation', 'restaurants.delete_reservation'],
    'live_orders': ['restaurants.view_order', 'restaurants.change_order'],
    'kitchen_display': ['users.access_to_view_kitchen_display'],
    'manage_kitchen': ['users.access_to_manage_kitchen_queue'],
    'billing': ['restaurants.view_invoice', 'restaurants.add_invoice', 'restaurants.change_invoice'],
    'payment_management': ['users.access_to_payment_management'],
    'categories': ['catalogs.view_category', 'catalogs.add_category', 'catalogs.change_category', 'catalogs.delete_category'],
    'items': ['catalogs.view_item', 'catalogs.add_item', 'catalogs.change_item', 'catalogs.delete_item'],
    'reports': ['users.view_reports'],
    'stores': ['stores.view_store', 'stores.add_store', 'stores.change_store', 'stores.delete_store'],
    'users': ['users.view_user', 'users.add_user', 'users.change_user', 'users.delete_user'],
    'manage_users': ['users.can_add_users'],
    'delete_order': ['users.access_to_delete_order'],
    'settings': ['core.view_taxconfiguration', 'core.change_taxconfiguration'],
}
